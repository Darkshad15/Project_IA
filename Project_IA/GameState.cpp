...
Menu::Menu()
    ...
    backgroundColor(sf::Color(255, 0, 0, 100)),
...
void Menu::setupMenu(const std::string& title, sf::Color titleColor, const std::vector<std::string>& options)
{
    ...
    if (!backgroundLoaded)
    {
        if (backgroundTexture.loadFromFile("../Assets/Stage_Layers/MainMenu.png"))
        {
            ...
            std::cout << "Background position: X=" << pos.x << " Y=" << pos.y << std::endl;
...
{
    std::cout << "Menu::draw() state = " << static_cast<int>(currentState) << std::endl;
    if (currentState == GameState::PLAYING)
        return;
    ...
    if (backgroundLoaded)
    {
        backgroundSprite.setColor(sf::Color::White); // 🔥 AVANT
